...
-- =====================================================
COMMENT
ON TABLE staging.fgislk_common_info IS 'Атрибутивная информация по лесничествам из ФГИС ЛК';
COMMENT
ON COLUMN staging.fgislk_common_info.region_code IS 'Код субъекта РФ';
COMMENT
ON COLUMN staging.fgislk_common_info.region_name IS 'Наименование субъекта РФ';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_district_code IS 'Код лесничества';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_district_name IS 'Наименование лесничества';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_quarter_code IS 'Номер квартала';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_plot_code IS 'Номер выдела';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_plot_area IS 'Площадь выдела, га';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_plot_characteristic IS 'Характеристика выдела';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_type IS 'Тип лесорастительных условий';
COMMENT
ON COLUMN staging.fgislk_common_info.dominant_species IS 'Преобладающая порода';
COMMENT
ON COLUMN staging.fgislk_common_info.age_class IS 'Класс возраста';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_group IS 'Группа лесов';
COMMENT
ON COLUMN staging.fgislk_common_info.forest_category IS 'Категория лесов';
COMMENT
ON COLUMN staging.fgislk_common_info.protection_category IS 'Категория защитности';
COMMENT
ON COLUMN staging.fgislk_common_info.purpose IS 'Целевое назначение лесов';
COMMENT
ON COLUMN staging.fgislk_common_info.inventory_date IS 'Дата проведения таксации';
COMMENT
ON COLUMN staging.fgislk_common_info.notes IS 'Примечания';
...
where ind = 'Киретское_1_22'

select *
from staging.forest_stand_kml_import fski
where ind = 'Киретское_1_6'
...
select *
from staging.forest_stand_kml_import fski
where ind = 'Окино-Ключевское_148_2'

select id, ind, index_field, kvart, vydel
from staging.forest_stand_kml_import
where ind <> index_field;
...
select *
from staging.forest_stand_kml_import fski
where ind = 'Окино-Ключевское_84_10'
...
select *
from staging.forest_stand_kml_import fski
where ind like 'Бичурское%'

select *
from staging.forest_stand_kml_import fski
where ind = 'Куналейское_0_null'

select count(*)
from staging.forest_stand_kml_import fski
where kate_1 is null
...
select distinct gir, lesni
from staging.forest_stand_kml_import

select distinct lesni, ind
from staging.forest_stand_kml_import

select distinct mu, gir, ugir
from staging.forest_stand_kml_import
...
create trigger trigger_forest_stand_area
    before
        insert
        or
update
...
ALTER
COLUMN geometry TYPE geometry(MULTIPOLYGON, 4326)
...
INSERT
INTO forest_stand (forestry_unit_id,
                   number_in_quarter,
                   full_number,
                   name,
                   description,
                   ...
                   created_at,
                   updated_at)
SELECT q.id                                                        AS forestry_unit_id,
       ...
       fi.vozra::integer AS age, fi.vysot::numeric(10,2) AS height, fi.diame::numeric(10,2) AS diameter, fi.bonit AS bonitet,
       fi.tip_l                                                    AS forest_type,
       fi.tlu                                                      AS tlu,
       fi.polno                                                    AS fullness,
       fi.zapas::numeric(10,2) AS stock, fi.kateg AS category,
       ...
       fi.plosha::numeric(10,2) AS area_ha, false AS verified,
       to_jsonb(fi.*)                                              AS source_data,
       EXTRACT(YEAR FROM NOW())::integer AS relevance_year, NOW() AS created_at,
       NOW()                                                       AS updated_at
FROM staging.forest_stand_kml_import fi
